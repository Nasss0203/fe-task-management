...
export function TaskStatusChart({ workspaceSlug, total, items }: TaskStatusChartProps) {
  ...
  const aggregatedItems = items.reduce((acc, item) => {
    ...
    return acc;
  }, [] as WorkspaceOverviewTaskStatusItem[]);

  const chartData = aggregatedItems.map((item, index) => ({
    ...item,
    value: item.count,
      ...
      <div className="mt-6 grid grid-cols-2 gap-x-4 gap-y-2">
        {chartData.map((item) => (
          <div key={item.statusId} className="flex items-center gap-2">
            ...
            <span className="ml-auto text-[11px] font-bold text-foreground flex items-center gap-1">
              {item.value}
