      ...
      >
                    ...
                    return (
                      ...
                      >
                        <ContextMenuTrigger asChild>
                          <div>
                            <TabsTriggerCustom value={item.value}>
                              <div className="flex items-center gap-1">
                                <item.icon />
        ...
        <div className={`mt-2 ${isOpen ? "mb-10" : ""}`}>
          {boards.length === 0 ? (
            context === "workspace" ? (
              <div className="ml-2 w-full max-w-3xl animate-in rounded-xl border border-dashed border-border/60 bg-background/30 p-4 fade-in duration-500">
                ...
                <div className="ml-8 flex flex-wrap gap-2">
                  <button
                    onClick={() =>
                      handleCreateBoard(BoardViewType.BOARD, "Theo trạng thái")
                    }
                  ...
                  <button
                    onClick={() =>
                      handleCreateBoard(BoardViewType.TABLE, "Bảng tính")
                    }
                    disabled={createBoard.isPending}
                    className="flex items-center gap-1.5 rounded-md border border-border/50 px-3 py-1.5 text-xs transition-colors hover:bg-accent hover:text-accent-foreground"
                  >
                  ...
                  <button
                    onClick={() =>
                      handleCreateBoard(BoardViewType.LIST, "Danh sách")
                    }
                    disabled={createBoard.isPending}
                    className="flex items-center gap-1.5 rounded-md border border-border/50 px-3 py-1.5 text-xs transition-colors hover:bg-accent hover:text-accent-foreground"
                  >
                  ...
                  <button
                    onClick={() =>
                      handleCreateBoard(BoardViewType.CALENDAR, "Lịch")
                    }
                    disabled={createBoard.isPending}
                    className="flex items-center gap-1.5 rounded-md border border-border/50 px-3 py-1.5 text-xs transition-colors hover:bg-accent hover:text-accent-foreground"
                  >
            ...
            <ActiveViewComponent board={activeBoard} context={context} />
          ) : null}
        </div>
      </Tabs>
    </div>
